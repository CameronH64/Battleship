...
public class BattleshipClient extends AbstractClient{
	...
	private JLabel labelMainMenuStatus;
	private JLabel labelCreateUserStatus;
	private JLabel labelDeleteUserStatus;
	private BattleshipClientGUI gui;
	...
	/*
	 * A copy of the previous constructor, but this one gets a GUI input to link labels and stuff
	 */
	public BattleshipClient(BattleshipClientGUI gui) {
		super("localhost", 8300);
	...
	public void handleMessageFromServer(Object arg0) {
		...
		if (arg0 instanceof MainMenuLoginData) {
			...
			{
				if (confirmation.getCreation()==true&&confirmation.getDeletion()==false)
				{
					//The user is on the Create User panel and is getting confirmation for creating account
					labelCreateUserStatus.setForeground(Color.GREEN);
					labelCreateUserStatus.setText("Successfully created account!");
				}
				else if (confirmation.getCreation()==false&&confirmation.getDeletion()==true)
				{
					//The user is on the Delete User panel and is getting confirmation for deleting account
					labelDeleteUserStatus.setForeground(Color.GREEN);
					labelDeleteUserStatus.setText("Successfully deleted account!");
				}
				else if (confirmation.getCreation()==false&&confirmation.getDeletion()==false)
				{
					//If credentials are valid, move to instructions screen
					CardLayout cardLayout = (CardLayout)container.getLayout();
					cardLayout.show(container, "4");
				}

			} 
			else //If credentials are invalid, display error message
			{
				//Check which panel is currently being used to determine response
				if (confirmation.getCreation()==true&&confirmation.getDeletion()==false) //User is on Create User panel
				{
					labelCreateUserStatus.setForeground(Color.RED);
					labelCreateUserStatus.setText("An account with that name already exists! Please choose another.");
				}
				else if (confirmation.getCreation()==false&&confirmation.getDeletion()==true) //User is on Delete User panel
				{
					labelDeleteUserStatus.setForeground(Color.RED);
					labelDeleteUserStatus.setText("Could not find that account! Please try again.");
				}
				else if (confirmation.getCreation()==false&&confirmation.getDeletion()==false) //User is on Main Menu panel
				{
					labelMainMenuStatus.setForeground(Color.RED);
					labelMainMenuStatus.setText("Invalid credentials. Please try again.");
				}
	...
	public JLabel getLabelMainMenuStatus() {
		return labelMainMenuStatus;
	}

	public void setLabelMainMenuStatus(JLabel labelMainMenuStatus) {
		this.labelMainMenuStatus = labelMainMenuStatus;
	}

	public JLabel getLabelCreateUserStatus() {
		return labelCreateUserStatus;
	}

	public void setLabelCreateUserStatus(JLabel labelCreateUserStatus) {
		this.labelCreateUserStatus = labelCreateUserStatus;
	}

	public JLabel getLabelDeleteUserStatus() {
		return labelDeleteUserStatus;
	}

	public void setLabelDeleteUserStatus(JLabel labelDeleteUserStatus) {
		this.labelDeleteUserStatus = labelDeleteUserStatus;
	}

	public BattleshipClientGUI getGui() {
		return gui;
	}

	public void setGui(BattleshipClientGUI gui) {
		this.gui = gui;
	}
